...
describe QbLineItemProvider do
  describe "get" do
    ...
    context "line item is in database" do
      ...
      context "line item has no name" do
        it "assumes the name is 'manual charge'" do
          space.line_items.should_receive(:find_by_description).with("manual charge").and_return(@item)
          qb_line_item = provider.get("")
        end
      end
    end
